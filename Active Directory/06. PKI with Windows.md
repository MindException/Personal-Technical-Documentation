...
여기서 추가적으로 궁금하게 되는 것은 서버 인증서는 웹서버로부터 받고 RootCA 인증서도 있으면,  
 ...
 실제로 IIS에서 호스팅하는 인증서를 확인하면 아래와 같이 출력됩니다. (_9번 목차 실습 예시_)  
...
* CryptoAPI가 AD에서 SubCA 인증서를 가져와 해당 웹서버 인증서하고 체인을 만드는게 매커니즘이다.
* 이를 IIS가 인증서로 호스팅하게 된다. (_발급 받은 인증서 .cer 파일 시점에는 SubCA 인증서 정보가 없다._)  


### ++ 추가) SubCA 인증서

대부분 2계층은 엔터프라이즈 CA로 하게 되며, 멤버 서버들에서 아래 명령어로 조회가 가능하다.  
SubCA의 경우 인증서가 변경되게 될 경우, AD에서 자동으로 배포하기 때문에 추가로 관리하지 않아도 된다.  

```powershell
C:\Users\leeadmin>certutil -store -enterprise CA
CA "중간 인증 기관"
================ 인증서 0 ================
일련 번호: 300000000bbbd3043de637e9dc00030000000b
발급자: CN=ROOTCA-01-CA
 NotBefore: 2026-05-02 오후 8:20
 NotAfter: 2027-05-02 오후 8:30
주체: CN=LSHCorp-LSHCA19-01-CA, DC=LSHCorp, DC=com
CA 버전: V1.1
인증서 템플릿 이름 (인증서 종류): SubCA
루트가 아닌 인증서
템플릿: SubCA, 하위 인증 기관
인증서 해시(sha1): 511bf41219c3e20a2b992ea3d8579d67fe2bb1ae
키 공급자 정보가 없습니다.
암호 해독에 대한 인증서와 개인 키를 찾을 수 없습니다.
CertUtil: -store 명령이 성공적으로 완료되었습니다.
```
* SubCA 인증서를 RootCA로부터 신규로 발급받은 이후에 SubCA CA 서비스를 껏다 키니 자동으로 배포되었다.  
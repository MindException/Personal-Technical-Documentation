...
링크1: [https://learn.microsoft.com/ko-kr/windows/win32/seccertenroll/certenroll-portal](https://learn.microsoft.com/ko-kr/windows/win32/seccertenroll/certenroll-portal)  
링크2: [https://learn.microsoft.com/ko-kr/windows-server/administration/windows-commands/certreq_1](https://learn.microsoft.com/ko-kr/windows-server/administration/windows-commands/certreq_1)

우선 CA로부터 인증서를 발급 받기 위해서는 `CSR` 파일을 만들어야 하며, 이때 CSR 파일 생성과 동시에  
Windows 내부에서는 해당 요청 값에 대한 `개인 키`가 생성되게 된다.  

Windows의 개인 키는 Linux에서 사용하는 개인 키와 같이 정말 키 값만 들어있지 않고, 하나의 개체 형태로 생성되게 된다.  
예를 들어서, 아래에 나올 `inf 파일`에 `Exportable = FALSE`로 설정을 하게 되면 해당 개인 키는 어떠한 일이 있어도 외부로 추출이 안된다.  

![](./MD_Images/09_00001.jpg)
* 따라서 초반에 inf 파일 설정이 중요하다.  
...
* __Subject__: 인증서의 주체를 넣습니다.
    * CN(Common Name): 인증서 이름이며, 주로 (_*.Corp.com으로 합니다._)
